...
class MyGame(arcade.Window):
    ...
    def on_resize(self, width, height):
        ...
        self.gravity = GRAVITY * self.scale_y
        self.jump_force = JUMP * self.scale_y

        self.rescale_sprites()

    def rescale_sprites(self):
        self.dino_jumping.scale = 3 * self.scale_y
        self.Spiky_Cactus.scale = 2 * 1.6 * self.scale_y
    ...
    def on_key_press(self, symbol: int, modifiers: int):
        if symbol == arcade.key.SPACE and not self.dino_jumping.in_jump_momentum:
            self.dino_jumping.change_y = self.jump_force
